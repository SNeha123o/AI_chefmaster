...
const Choices = ({Country}) => {
  ...
  return (
    <div className='bg-zinc-900 text-white flex flex-col justify-center items-center py-8'>
        ...
        <h1 className='text-3xl font-semibold pb-8 text-yellow-500'> {!Country ? ` What do you want to cook` : `Famous ${Country} Dishes`}</h1>
        ...
        <h1 className=' mx-24 ml-[11rem] mr-[10rem] mt-4 text-3xl font-semibold pb-8 text-yellow-500'> {!Country ? `` : `Unleash the joy of discovering new tastes of ${Country} Cuisine , celebrating the diversity that makes ${Country} cuisine an enchanting feast for the senses.`}</h1>

        {/* CUISINE */}
    
        {!Country && (
        <><h1 className='text-3xl font-semibold py-8 text-yellow-500'>{!Country ? `Choices` : ``}</h1>
        <div className="card-container grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6  gap-4 lg:gap-16 items-center justify-center">
